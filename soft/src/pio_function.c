...
void Timer_init(uint32_t load_value){
    // Load the timer count value to the load count register
    OSC1_0_REG(OSC1_LOAD_OFFSET) = load_value;

    // Set every bit of ctl reg to 0 (Disbles timer)
    OSC1_0_REG(OSC1_CTLREG_OFFSET) &= ~0x7;

    // bit 1 to 1 (Sets mode to user count)
    OSC1_0_REG(OSC1_CTLREG_OFFSET) |= 0x2;

    #ifdef DEBUG_PIO
        printf("[pio_function] timer control reg : %#X\n", OSC1_0_REG(OSC1_CTLREG_OFFSET));
    #endif    
}

// Timer_start function : Start the timer
void Timer_start(void){
    // Enable timer (set 1 to bit 0)
    OSC1_0_REG(OSC1_CTLREG_OFFSET) |= 0x1;
}

// Timer_stop function : Stop the timer
void Timer_stop(void){
    // Disable timer (set 0 to bit 0)
    OSC1_0_REG(OSC1_CTLREG_OFFSET) &= ~0x1;
}

// Timer_get_value function : Get the current value of the timer
uint32_t Timer_get_value(void){
    uint32_t value = OSC1_0_REG(OSC1_VAL_OFFSET);
